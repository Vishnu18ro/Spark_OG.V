...
import { OPVProfileInfoPreview } from "@/components/portfolio/OPVProfileInfoPreview";
import { OPVProjectsPreview } from "@/components/portfolio/OPVProjectsPreview";
import { OPVSurpriseProjectsPreview } from "@/components/portfolio/OPVSurpriseProjectsPreview";
import { OPVSkillsPreview } from "@/components/portfolio/OPVSkillsPreview";
import { OPVCertificatesPreview } from "@/components/portfolio/OPVCertificatesPreview";
import { OPVExperiencePreview } from "@/components/portfolio/OPVExperiencePreview";
import { OPVCodingPlatformPreview } from "@/components/portfolio/OPVCodingPlatformPreview";
import { OPVAboutMePreview } from "@/components/portfolio/OPVAboutMePreview";
import { OPVEducationPreview } from "@/components/portfolio/OPVEducationPreview";
import { OPVPersonalConnectionPreview } from "@/components/portfolio/OPVPersonalConnectionPreview";
import { logout, isGuestMode, enableGuestMode } from "@/lib/auth";
...
export default function OPVPortfolioDetail() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { profileImage, username, session, isLoading } = useProfile();
  const { opvUserData, isOPVMode, setOpvUserData, setIsOPVMode } = useOPV();
  ...
  useEffect(() => {
    if (!isLoading && !session && !isGuestMode() && !routeUsername) {
      navigate("/");
    }
  }, [session, isLoading, navigate, routeUsername]);

  // Handle URL-based hydration
  useEffect(() => {
    const fetchUserByUsername = async (usernameInput: string) => {
      ...
      if (data) {
        setOpvUserData(data);
        setIsOPVMode(true);
        // Enable guest mode if not logged in, so deep linking works for fresh sessions
    ...
    if (routeUsername) {
      // If URL has username, ensure context matches or fetch it
      if (!opvUserData || opvUserData.username !== routeUsername) {
        fetchUserByUsername(routeUsername);
      } else {
        setIsOPVMode(true); // Ensure mode is set if we have data
      }
    } else if (opvUserData?.username) {
      // If no URL param but we have data, update URL to include username
      // Use replace to avoid history stack buildup
      navigate(`/opv-portfolio/${opvUserData.username}?${searchParams.toString()}`, { replace: true });
    } else {
      // No param and no data -> go back to OPV search
      navigate("/opv");
    }
  }, [routeUsername, opvUserData, navigate, setOpvUserData, setIsOPVMode, searchParams]);
      ...
      <div className="pt-20 flex w-full">
        ...
        <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? "lg:ml-64" : "lg:ml-16"
          ...
          <div className="container mx-auto px-4 py-8 space-y-8" style={{ zoom: "75%" }}>
            ...
            <div id="certificates">
              <OPVCertificatesPreview />
            </div>

            <div id="experience">
              <OPVExperiencePreview />
            </div>

            <div id="coding-platform">
              <OPVCodingPlatformPreview />
            </div>
