...
export function OPVProfileInfoPreview() {
  const { opvUserData } = useOPV();
  const profile = (opvUserData?.profile as any) || {};
  const profileImage = profile.image?.value || profile.image;
  const name = profile.name;
  const tagline = profile.tagline;
  const githubUrl = profile.github?.value || profile.github;
  const linkedinUrl = profile.linkedin?.value || profile.linkedin;
  ...
  return (
    <div id="opv-profile-info-section" className="bg-card/50 backdrop-blur-sm border border-primary/30 rounded-lg p-8 shadow-glow-primary">
