...
const scrollToProfileInfo = () => {
  const profileInfoSection = document.getElementById('opv-profile-info-section');
  if (profileInfoSection) {
    profileInfoSection.scrollIntoView({ behavior: 'smooth' });
  }
};

export function OPVPersonalConnectionPreview() {
  const { opvUserData } = useOPV();
  const navigate = useNavigate();
  const [isStartingDM, setIsStartingDM] = useState(false);

  const personalConnect = (opvUserData?.personal_connect as any) || {};
  const email = personalConnect.email;
  const location = personalConnect.location;

  const handleDirectMessage = async () => {
    if (!opvUserData?.auth_user_id) {
    ...
    try {
      ...
      // Navigate to messages with the recipient's auth_user_id
      navigate(`/messages/${opvUserData.auth_user_id}`);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        {/* RIGHT SIDE - Note about contact form */}
        <div className="space-y-4">
          <div>
          ...
          <button
            ...
            disabled={isStartingDM || isGuestMode()}
            className="w-full mt-4 p-4 rounded-xl border border-purple-500/30 bg-[#0d0d0d]/50 backdrop-blur-sm hover:border-purple-500/50 hover:shadow-[0_0_25px_rgba(168,85,247,0.4)] transition-all group cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-sm font-medium text-gray-200 group-hover:text-purple-300 transition-colors">
                  {isStartingDM ? "Starting conversation..." : "Direct Message"}
                </p>
